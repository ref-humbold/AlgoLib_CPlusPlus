...
std::vector<int> alma::find_primes(size_t max_number)
{
...
std::vector<int> alma::find_primes(size_t min_number, size_t max_number)
{
    std::vector<int> primes;
    std::vector<bool> is_prime, base_primes;
    int diff = max_number - min_number;

    for(size_t i = 0; i <= sqrt(max_number); ++i)
